...
public class Smote extends Operator {
	...
	private ExampleSet apply(ExampleSet originalSet) throws OperatorException {
		...
		MemoryExampleTable smotedTable = MemoryExampleTable
				.createCompleteCopy(originalSet.getExampleTable());
		int resultSize = 0;

		label = originalSet.getAttributes().getLabel();
		if (label != null) {
			if (label.isNominal()) {
				filter = new AttributeValueFilter(label,
		...
		DistanceMeasure measure = measureHelper
				.getInitializedMeasure(originalSet);

		GeometricDataCollection<double[]> samples = new LinearList<double[]>(
				measure);
		...
		for (Example example : minoritySet) {
			...
			Collection<double[]> neighborValues = samples.getNearestValues(k,
					values);
			// I hate Java
			...
			while (it.hasNext()) {
				...
				for (int i = 0; i < 4; i++) {
					if (debug)
						System.out.println(neighborArray[index][i]);
				}
				index++;
			}
			...
			for (int j = 0; j < n; j++) {
				...
				double[] newDataRowData = new double[smotedTable
				...
				DataRow synthetic = smotedTable
						.getDataRow(smotedTable.size() - 1);
				// get random neighbor
				double[] randomNeighborValues = neighborArray[generator
						.nextInt(neighborValues.size() - 1)];
				if (debug)
					System.out.println("Chosen");
				for (int i = 0; i < 4; i++) {
					if (debug)
						System.out.println(randomNeighborValues[i]);
				}
				// for each continuous attribute
				index = 0;
				for (Attribute attribute : attributes) {
					// calculate distance
					double distance = Math.abs(randomNeighborValues[index]
							- values[index]);
					// generate random gap
					...
					if (debug)
						System.out.println(randomNeighborValues[index]);
					if (debug)
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeInt(
				PARAMETER_SMOTE_N,
				"Amount of Smote N%. The amount of SMOTE is assumed to be integral multiplies of 100.",
				0, Integer.MAX_VALUE, 300);
		ParameterType type2 = new ParameterTypeInt(
				PARAMETER_NEAREST_NEIGHBORS,
				"Numbers of nearest neighbors taken into consideration when constructing synthetic examples.",
				0, Integer.MAX_VALUE, 3);
		ParameterType type3 = new ParameterTypeString(
				PARAMETER_MINORITY_CLASS_NAME, "Minority class name", false);
